...
# ---- SP-2 2c dungeon/collection quest specifics ----
OUT_DIR = os.path.join("npc", "backport", "re_collections")
# Dungeon/collection quest files (rathena-ref npc/re/quests). Plain file list.
COLLECTION_FILES = ["magic_books.txt", "homun_s.txt", "illusion_investigation.txt",
                    "quests_illusion_dungeons.txt", "juno_monster_society.txt",
                    "garden_of_time.txt", "quests_dungeons_200.txt", "HelpMeShorty.txt"]
HEADER = ("//===== uAthena backport (renewal dungeon/collection quests 2c, GENERATED) =====\n"
          "//= dungeon/collection quests from rathena-ref; tokens adapted, renewal-feature blocks commented.\n"
          "//= DO NOT EDIT - regenerate via dumps/forge/backport-renewal-collections.py\n"
          "//= Source: {src}\n"
          "//============================================================\n")

def quest_sources():
    return [f"npc/re/quests/{n}" for n in COLLECTION_FILES]
...
def gen():
    gap_buf = ["# Renewal 2c dungeon/collection gap-лог (адаптации/dedup/boundary/unresolved/orphan)\n\n"]
    ...
    for name in bodies:
        ...
        write_raw(os.path.join(UA_ROOT, OUT_DIR, name),
                  ...
                  ("" if body.endswith("\n") else "\n"))
        includes.append(f"npc: npc/backport/re_collections/{name}")
        if ev:
            gap_buf.append(f"\n## {name}\n")
    ...
    write(os.path.join(UA_ROOT, "dumps", "forge", "backport-renewal-collections-maps.txt"),
          "\n".join("map: " + m for m in scope_maps) + "\n")
    write(os.path.join(UA_ROOT, "Doc", "backport_renewal_collections_gap.md"), "".join(gap_buf))
    write(os.path.join(UA_ROOT, "dumps", "forge", "backport-renewal-collections-includes.txt"),
          "\n".join(includes) + "\n")
