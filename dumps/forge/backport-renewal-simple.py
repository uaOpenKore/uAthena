...
# ---- SP-2 2d simple quest specifics ----
OUT_DIR = os.path.join("npc", "backport", "re_simple")
# Simple/misc quest files (rathena-ref npc/re/quests). Plain file list. Several are tiny
# renewal duplicate-only stubs (base NPC live elsewhere -> orphan-pass handles missing bases).
SIMPLE_FILES = ["ninja_quests.txt", "cupet.txt", "cooking_quest.txt", "pile_bunker.txt",
                "mrsmile.txt", "monstertamers.txt", "the_sign_quest.txt", "quest_payon.txt"]
HEADER = ("//===== uAthena backport (renewal simple quests 2d, GENERATED) =====\n"
          "//= simple/misc quests from rathena-ref; tokens adapted, renewal-feature blocks commented.\n"
          "//= DO NOT EDIT - regenerate via dumps/forge/backport-renewal-simple.py\n"
          "//= Source: {src}\n"
          "//============================================================\n")

def quest_sources():
    return [f"npc/re/quests/{n}" for n in SIMPLE_FILES]
...
def gen():
    gap_buf = ["# Renewal 2d simple gap-лог (адаптации/dedup/boundary/unresolved/orphan)\n\n"]
    ...
    for name in bodies:
        ...
        write_raw(os.path.join(UA_ROOT, OUT_DIR, name),
                  ...
                  ("" if body.endswith("\n") else "\n"))
        includes.append(f"npc: npc/backport/re_simple/{name}")
        if ev:
            gap_buf.append(f"\n## {name}\n")
    ...
    write(os.path.join(UA_ROOT, "dumps", "forge", "backport-renewal-simple-maps.txt"),
          "\n".join("map: " + m for m in scope_maps) + "\n")
    write(os.path.join(UA_ROOT, "Doc", "backport_renewal_simple_gap.md"), "".join(gap_buf))
    write(os.path.join(UA_ROOT, "dumps", "forge", "backport-renewal-simple-includes.txt"),
          "\n".join(includes) + "\n")
...
def main():
    if "--selftest" in sys.argv:
        assert len(quest_sources()) == 8, quest_sources()
        assert all(s.startswith("npc/re/quests/") and "/eden/" not in s for s in quest_sources())
        assert "ninja_quests.txt" in SIMPLE_FILES and "quest_payon.txt" in SIMPLE_FILES
        assert "DUPLICATE" not in GAP_BUILTINS
