...
# ---- SP-2 2b Eden Group quest specifics ----
OUT_DIR = os.path.join("npc", "backport", "re_eden")
# Eden Group quest files (rathena-ref npc/re/quests/eden). Plain file list (no zone pattern).
# Order: shared deps first (common/service), then tutorial, then board/level quests.
EDEN_FILES = ["eden_common.txt", "eden_service.txt", "eden_tutorial.txt", "eden_quests.txt",
              "eden_iro.txt", "11-25.txt", "26-40.txt", "41-55.txt", "56-70.txt", "71-85.txt",
              "86-90.txt", "91-99.txt", "eden_100_111.txt", "eden_111_120.txt",
              "eden_121_130.txt", "eden_131_140.txt"]
HEADER = ("//===== uAthena backport (renewal Eden Group quests 2b, GENERATED) =====\n"
          "//= Eden quests from rathena-ref; tokens adapted, renewal-feature blocks commented.\n"
          "//= DO NOT EDIT - regenerate via dumps/forge/backport-renewal-eden.py\n"
          "//= Source: {src}\n"
          "//============================================================\n")

def quest_sources():
    return [f"npc/re/quests/eden/{n}" for n in EDEN_FILES]
...
def gen():
    gap_buf = ["# Renewal 2b Eden-quest gap-лог (адаптации/dedup/boundary/unresolved/orphan)\n\n"]
    ...
    for name in bodies:
        ...
        write_raw(os.path.join(UA_ROOT, OUT_DIR, name),
                  HEADER.format(src="npc/re/quests/eden/" + name) + body +
                  ("" if body.endswith("\n") else "\n"))
        includes.append(f"npc: npc/backport/re_eden/{name}")
        if ev:
            gap_buf.append(f"\n## {name}\n")
    ...
    write(os.path.join(UA_ROOT, "dumps", "forge", "backport-renewal-eden-maps.txt"),
          "\n".join("map: " + m for m in scope_maps) + "\n")
    write(os.path.join(UA_ROOT, "Doc", "backport_renewal_eden_gap.md"), "".join(gap_buf))
    write(os.path.join(UA_ROOT, "dumps", "forge", "backport-renewal-eden-includes.txt"),
          "\n".join(includes) + "\n")
...
def main():
    if "--selftest" in sys.argv:
        assert len(quest_sources()) == 16, quest_sources()
        assert all(s.startswith("npc/re/quests/eden/") for s in quest_sources())
        assert "DUPLICATE" not in GAP_BUILTINS
