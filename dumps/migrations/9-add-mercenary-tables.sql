-- [Backport] Hired Mercenary Soldier storage.
-- `mercenary`       — one row per summoned mercenary instance (mer_id auto-increment).
-- `mercenary_owner` — per-character mer_id + guild loyalty/calls (loaded with the char).
