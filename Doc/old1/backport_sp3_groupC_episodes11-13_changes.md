...
> **⚠️ ОБНОВЛЕНИЕ (важно):** из 8 файлов группы C пять (ein/hugel/juperos/lighthalzen/rachel) уже были у
> uAthena (старые версии без quest-log, регистрировались сами) — по решению они **АПГРЕЙЖЕНЫ** на eathena
> quest-log версии (старые регистрации отключены в `scripts_athena.conf`). Три (veins/13_1/13_2) — новые.
> Это не чисто аддитивно для тех пяти. **Тестировщикам:** проверить перекрёстные ссылки на апгрейженные
> квест-NPC (`donpcevent`/`doevent` — рантайм, boot не ловит). Коммит апгрейда: `f2ff1b4`.
