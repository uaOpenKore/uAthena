...
## Фаза M7d-5b — AI активирует self-бафы (СДЕЛАНО)

Замыкает картину combat-наёмника: теперь AI не только бьёт скиллами, но и сам поддерживает свои
self-бафы (механика которых подключена в M7d-3a).

**Что изменилось (`src/map/mercenary_soldier.c`):**
- Хелпер `merc_use_self_buffs()`: проходит список скиллов наёмника, для каждого self-таргет-бафа
  (`INF_SELF_SKILL`) проверяет через `SkillStatusChangeTable()`, активен ли его SC; если нет — кастует
  на себя (`unit_skilluse_id` в `md->bl.id`). По одному бафу за вызов (время каста гейтит следующий) →
  баф не спамится и обновляется при истечении.
- Интеграция в `merc_ai_sub_hard`: когда наёмник простаивает рядом с мастером (нет цели), он добивает
  отсутствующий self-баф. Так бафы поднимаются на спавне (наёмник появляется рядом с мастером) и
  обновляются между боями. Гейт — тот же `merc_skill_rate>0` (0 отключает и каст, и бафы).

**Поведение:** буфаются PARRYING/REFLECTSHIELD/BERSERK/DEFENDER/AUTOGUARD/QUICKEN/AUTOBERSERK/SIGHT (по
классу наёмника). SC-проверка не даёт перекастовывать активный баф. В долгом бою истёкший баф обновится,
когда наёмник вернётся в idle у мастера (MVP — не рефрешит прямо в бою).

**Верификация:** `make sql` EXIT=0, 0 новых ворнингов. Runtime — на тестировщиках.

**Итог combat-наёмника:** офенс-скиллы в бою (M7d-5) + self-бафы в idle (M7d-5b) = полноценный боевой
наёмник. Осталось (по типам наёмников): **M7d-3b** (саппорт/cure/devotion на мастера — для support-мерка),
**M7d-4** (ловушки — для archer-мерка).

**Откат:** `merc_skill_rate: 0` или реверт коммита.

<!-- M7d-3b (саппорт/cure/devotion на мастера), M7d-4 (ловушки) — далее. -->
