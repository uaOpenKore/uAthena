...
//////////////////////////////////////
	...
	int sig_init() {
		ShowError("This plugin is not supported - Enable 'exchndl' instead!");
		return 0;
	}
	int sig_final() { return 0; }
#elif defined (__NETBSD__) || defined (__FREEBSD__)
	int sig_init() {
		ShowError("This plugin is not supported!");
		return 0;
	}
	int sig_final() { return 0; }
#else
...
void sig_dump(int sn)
{
	...
	if ((fp = FOPEN_(file, "w", stderr)) != NULL) {
		...
		ShowNotice ("Dumping stack to '"CL_WHITE"%s"CL_RESET"'... ", file);
