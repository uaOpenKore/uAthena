...
/*==========================================
 * ©ªð?bNµĂ¢éMOBÌ?ð?¦é(foreachclient)
 *------------------------------------------
 */
...
int battle_damage(struct block_list *src,struct block_list *target,int damage, int flag)
{
	...
	nullpo_retr(0, target); //srcÍNULLŌĂ΂ê邱Ƃª é̂ő¼Ń`FbN
...
{
	nullpo_retr(0, target); //blÍNULLŌĂ΂ê邱Ƃª é̂ő¼Ń`FbN
...
int battle_attr_fix(struct block_list *src, struct block_list *target, int damage,int atk_elem,int def_elem)
{
	...
	if (atk_elem < 0 || atk_elem > 9)
		atk_elem = rand()%9;	//?푮?«_ŕtÁ

	if (def_type < 0 || def_type > 9 ||
		def_lv < 1 || def_lv > 4) {	// ® ?«lª¨©µ¢̂łƂ肠¦¸»̂܂ܕԂ·
	...
	switch(weapon)
	{
		...
		case 0x05:	// 2HL
		{
			...
			if((skill = pc_checkskill(sd,KN_SPEARMASTERY)) > 0) {
				if(!pc_isriding(sd))
					damage += (skill * 4);	// yRÉ?æÁĂȂ¢
				else
					damage += (skill * 5);	// yRÉ?æÁĂé
			}
			break;
		}
		case 0x06: // 1H Axe
		...
		if (skill_num && !flag.hit)
			switch(skill_num)
			{
				case AS_SPLASHER: //Reports say it always hits?
					if (wflag&1) //Only if you were the one exploding.
						break;
	 ...
	 unsigned int tick,int flag)
{
	...
	if (sd) {
		if (wd.flag & BF_WEAPON && src != target && damage > 0) {
			int hp = 0, sp = 0;
			if (!battle_config.left_cardfix_to_right) { // ñ?¬?¶èJ?[h̋zûnøʂðEèɒǉÁµȂ¢?ê?
			...
			} else { // ñ?¬?¶èJ?[h̋zûnøʂðEèɒǉÁ·é?ê?
...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	if (map_flag_vs(m)) { //Check rivalry settings.
		...
		if (state&BCT_ENEMY && battle_config.pk_mode && !map_flag_gvg(m) &&
			s_bl->type == BL_PC && t_bl->type == BL_PC && !is_duel)
		...
		if (flag&BCT_PARTY || state&BCT_ENEMY) {
			int s_party = status_get_party_id(s_bl);
			if (!is_duel && s_party && s_party ==status_get_party_id(t_bl))
				state |= BCT_PARTY;
		}
		if (flag&BCT_GUILD || state&BCT_ENEMY) {
			int s_guild = status_get_guild_id(s_bl);
			int t_guild = status_get_guild_id(t_bl);
			if (!is_duel && s_guild && t_guild && (s_guild == t_guild || guild_idisallied(s_guild, t_guild)))
				state |= BCT_GUILD;
		}
	}
