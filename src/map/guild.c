...
// ƒƒOƒCƒ“’†‚ÌƒMƒ‹ƒhƒƒ“ƒo[‚Ì‚Pl‚Ìsd‚ğ•Ô‚·
...
// ƒMƒ‹ƒhƒƒ“ƒo[‚ÌƒCƒ“ƒfƒbƒNƒX‚ğ•Ô‚·
...
int guild_recv_info(struct guild *sg)
{
	...
	for(i=bm=m=0;i<g->max_member;i++){	// sd‚Ìİ’è‚Æl”‚ÌŠm”F
	...
	for(i=0;i<g->max_member;i++){	// î•ñ‚Ì‘—M
		...
		if( sd->state.guild_sent==0){	// –¢‘—M‚È‚çŠ‘®î•ñ‚à‘—‚é
...
int guild_reqalliance(struct map_session_data *sd,int account_id)
{
	...
	if( tsd->guild_alliance>0 ){	// ‘Šè‚ª“¯–¿—v¿ó‘Ô‚©‚Ç‚¤‚©Šm”F
